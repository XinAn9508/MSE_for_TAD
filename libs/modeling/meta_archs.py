...
@register_meta_arch("MSE_TAD")
class MSE_TAD(nn.Module):
    """
    ...
    def __init__(
            ...
            n_head,
            n_mse_win_size,  # window size w for mse
            ...
            k,  # the K in mse
            init_conv_vars,  # initialization of gaussian variance for the weight in mse
        ...
        if isinstance(n_mse_win_size, int):
            self.mse_win_size = [n_mse_win_size] * len(self.fpn_strides)
        else:
            assert len(n_mse_win_size) == len(self.fpn_strides)
            self.mse_win_size = n_mse_win_size
        max_div_factor = 1
        for l, (s, w) in enumerate(zip(self.fpn_strides, self.mse_win_size)):
        ...
        assert backbone_type in ['MSEBackbone']
        ...
        self.backbone = make_backbone(
            ...
            **{
                ...
                'mse_win_size': self.mse_win_size,
