...
class MaskedMHCA(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        k = k.view(B, self.n_head, self.n_channels, -1).transpose(2, 3) #[2,4,2304,128]
        q = q.view(B, self.n_head, self.n_channels, -1).transpose(2, 3) #[2,4,2304,128]
        v = v.view(B, self.n_head, self.n_channels, -1).transpose(2, 3) #[2,4,2304,128]

        # self-attention: (B, nh, T', hs) x (B, nh, hs, T'') -> (B, nh, T', T'') [2,4,2304,128] x [2,4,128,2304] -> [2,4,2304,2304]
        att = (q * self.scale) @ k.transpose(-2, -1) #[2,4,2304,2304]
        # prevent q from attending to invalid tokens
        att = att.masked_fill(torch.logical_not(kv_mask[:, :, None, :]), float('-inf'))
        # softmax attn
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att) #[2,4,2304,2304]
        # (B, nh, T', T'') x (B, nh, T'', hs) -> (B, nh, T', hs)  #[2,4,2304,2304] x [2,4,2304,128] -> [2,4,2304,128]
        out = att @ (v * kv_mask[:, :, :, None].to(v.dtype)) #[2,4,2304,128]
        # re-assemble all head outputs side by side
        out = out.transpose(2, 3).contiguous().view(B, C, -1) #[2,512,2304]
...
class MSEBlock(nn.Module):
    ...
    def __init__(
        ...
        # self.attn_psi = nn.Conv1d(n_embd, n_embd, kernel_size, stride=1, padding=kernel_size // 2, groups=n_embd)
        ...
        self.dynamic_weight = nn.Parameter(torch.ones(6, requires_grad=True)) #DynamicWeight
    ...
    def forward(self, x, mask):
        ...
        # global 
        # attn_out, attn_out_mask = self.attn(self.gn(x), mask)########################
        ...
        # attn_out = out * out_mask + self.drop_path_attn(attn_out)#################
        ...
        ##################
        out1 = self.dynamic_weight[0]*fc * phi + self.dynamic_weight[1]*(convw + convkw ) * psi + self.dynamic_weight[2]*out
        attn_out, attn_out_mask = self.attn(self.ln(out1), out_mask)
        attn_out = self.dynamic_weight[3]*out1 + self.dynamic_weight[4]*out + self.dynamic_weight[5]*attn_out

        attn_out = x * attn_out_mask + self.drop_path_out(attn_out)
        attn_out = attn_out + self.drop_path_mlp(self.mlp(self.gn(attn_out)))
        return attn_out, attn_out_mask.bool()
